...
{
    public class OddEvenStrategy : TournamentServiceBase, ICaculationResultStrategy
    {
        private readonly IMatchesStatisticStrategy _matches;

        public OddEvenStrategy(ILogger<OddEvenStrategy> logger, TournamentDbContext dbContext, [FromKeyedServices("matches")] IMatchesStatisticStrategy matches)
    ...
    .FirstOrDefaultAsync(x => x.MinigameID == minigameID);
            ...
            bool? OddEven = (totalScoreA + totalScoreB) % 2 == 0 ? false : true;

            bool? OddEvenH1 = (teamAScoreH1 + teamBScoreH1) % 2 == 0 ? false : true;

            bool? OddEvenH2 = (teamAScoreH2 + teamBScoreH2) % 2 == 0 ? false : true;
