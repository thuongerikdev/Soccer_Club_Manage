...
{
    internal class TournamentMatchService : TournamentServiceBase, IMatchesStatisticStrategy
    {
        private readonly IMatchStatBase _matchStatBase;
        public TournamentMatchService(ILogger<TournamentMatchService> logger, TournamentDbContext dbContext, IMatchStatBase matchStatBase) : base(logger, dbContext)
        {
        ...
        public async Task<TournamentResponeDto> getStatisTic(ReadMatchesStatisticDto readMatchesStatisticDto)
        {
            ...
            foreach (var item in listmatch)
            {
                var matchStats = await _dbContext.MatchesStatistics
                    .Where(x => x.MatchesID == item.MatchesID)
                    .ToListAsync();
